...
class Splint:
    ...
    def buildP1(self):
        try:
            ...
            if not profTwo == None:
                ui.messageBox("here")
            ui.messageBox(str(profSelect.count))
            ...
            #     points[i] = addPoint(points[i], adsk.core.Point3D.create(-(self._connectLength - defaultConnectLength),0,0))
            for i in range(len(points)):
    ...
    def buildP2(self):
        try:
            ...
            #extrude2
            strapHoleEx = adsk.core.ValueInput.createByReal(-self._subRadius)
            strapProfile = xYSketch.profiles.item(0)
            ext2 = extrudeFeatures.addSimple(strapProfile, strapHoleEx, adsk.fusion.FeatureOperations.CutFeatureOperation)
            ...
            ext2 = extrudeFeatures.addSimple(selectProfile, strapHoleEx, adsk.fusion.FeatureOperations.CutFeatureOperation)
    ...
    def assemble(self):
        try:
            ...
            selectEdgeOne = None
            # for edge in self._ref1.edges:
            #     if edge.geometry.curveType == 2:
            #         selectEdgeOne = edge
            #         break
            # selectEdgeTwo = None
            # for edge in self._ref2.edges:
            #     if edge.geometry.curveType == 2:
            #         selectEdgeTwo = edge
            #         break
            #ui.messageBox(str(self._ref2.objectType))
            # sketches = rootComponent.sketches
            # features = rootComponent.features
            # planes = self.comp2.constructionPlanes
            # extrudeFeatures = features.extrudeFeatures
            # planeInput = planes.createInput()
            # offsetValue = adsk.core.ValueInput.createByReal(-1.65)
            # planeInput.setByOffset(rootComponent.xZConstructionPlane, offsetValue)
            # sketchNPlane = planes.add(planeInput)
            # sketchN = sketches.add(sketchNPlane)
            # selectFaceMain = []
            # projectedEntities = sketchN.intersectWithSketchPlane(self._ref2)
            #extrudeFeatures.addSimple(self._ref2, adsk.core.ValueInput.createByReal(0.6), adsk.fusion.FeatureOperations.NewBodyFeatureOperation)
            if self._ref1 == None:
                ui.messageBox("here")
